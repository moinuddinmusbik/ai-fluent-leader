...
Every routine email MUST use an **AI-Fluent Leader** branded HTML template — **Georgia serif**, `is_html=true`. There are three layouts: the **Daily Brief** (Nate B Jones — strategy lane) skeleton below, the **Implementation Playbook** (Nate Herk — implementation lane) skeleton after it, and the **Weekly Digest** (Weekly AI Leadership Stories) skeleton further down. Do NOT invent a new inline style per run; consistency across routines is the point.
...
- Nate B Jones (Daily Brief): `AI-Fluent Leader Daily Brief`
- Nate Herk (Implementation Playbook): `AI-Fluent Leader · Implementation Playbook`
...
Nate Herk's lane is **hands-on implementation**, so his email is a **reproducible playbook**, not a strategy brief — and it is built from his **actual transcript** (spoken words), not just the title/description. It reuses the Daily Brief card chrome (dark header `rgb(26,26,46)`, Georgia, 700px card, red Mistake heading, highlighted action block) and adds four things the Daily Brief lacks:
...
The "Steal This For Your Org" action block is highlighted `rgb(248,249,255)`; the "Mistake to Avoid" heading is red `rgb(192,57,43)`; links `rgb(74,108,247)`. Footer identical to the Daily Brief.
