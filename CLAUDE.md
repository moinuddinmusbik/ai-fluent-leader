...
Every routine email MUST use an **AI-Fluent Leader** branded HTML template — **Georgia serif**, `is_html=true`. There are three layouts: the **Strategy Brief** (Nate B Jones — strategy lane) skeleton below, the **Implementation Playbook** (Nate Herk — implementation lane) skeleton after it, and the **Weekly Digest** (Weekly AI Leadership Stories) skeleton further down. Do NOT invent a new inline style per run; consistency across routines is the point.

### Strategy Brief — design tokens (Nate B Jones)

Nate B. Jones's lane is **executive strategy and analysis**, so his email must do justice to his **viewpoint and reasoning** — not a list of topics. It is built from his **actual transcript** (spoken words, pulled via Firecrawl) plus his linked **Substack post** (the free preview; the full post is usually paywalled), and it foregrounds his *argument*, the *frameworks* he names, the *evidence/receipts* he cites, and where he *breaks from consensus*. It reuses the shared card chrome (dark header `rgb(26,26,46)`, Georgia, 700px card, highlighted action block, red Mistake heading) and adds two things the old Daily Brief lacked:

1. **A lens badge** in the header — `TAKE`, `FRAMEWORK`, or `REPORT` (classification below). The section set adapts to the lens.
2. **An "In His Words" block** — 1–2 verbatim transcript/Substack quotes with timestamps. This is the "actual words" signal; every brief carries at least one real quote.

Tokens:
- Page background `rgb(244,244,244)`; card `max-width:700px; border-radius:8px; overflow:hidden`.
- Header band `background:rgb(26,26,46)`; eyebrow `11px, letter-spacing:2px, uppercase, rgb(136,136,170)`; H1 `22px, rgb(255,255,255)`; meta line `13px, rgb(170,170,204)` with the date in `<strong style="color:rgb(224,224,255)">` + a lens badge pill.
...
- The "What It Means For You" action block is highlighted `background:rgb(248,249,255)`. The "Mistake to Avoid" heading is red `rgb(192,57,43)`. Links `rgb(74,108,247)`.
...
**Lens classification (pick ONE from the transcript + title):**

| Lens | When | Section order |
|------|------|---------------|
| **TAKE** | His analysis/opinion on a release, event, or trend (most common) | The Big Idea → Why It Matters Now → His Argument → Receipts → The Contrarian Edge → What It Means For You → Mistake to Avoid → In His Words → Source |
| **FRAMEWORK** | He teaches a mental model / framework leaders should adopt | The Big Idea → The Framework → How It Works → Where It Applies → Receipts → What It Means For You → Mistake to Avoid → In His Words → Source |
| **REPORT** | He breaks down a study/report (McKinsey, GDPval, Gartner, WEF…) | The Big Idea → What The Report Says → Nate's Read → Receipts → The Contrarian Edge → What It Means For You → Mistake to Avoid → In His Words → Source |

**Strategy Brief HTML skeleton.** Assemble the **header chrome**, then the ordered **section partials** for the chosen lens, then close. Fill `{{PLACEHOLDERS}}`; repeat inner `<li>` as needed.
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:rgb(244,244,244);padding:32px 0;font-family:Georgia,serif"><tbody><tr><td align="center">
  <table width="700" cellpadding="0" cellspacing="0" style="max-width:700px;border-radius:8px;overflow:hidden;font-family:Georgia,serif"><tbody>
    <tr><td style="background:rgb(26,26,46);padding:28px 36px 22px">
      <p style="margin:0 0 6px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:rgb(136,136,170)">AI-Fluent Leader · Strategy Brief</p>
      <h1 style="margin:0 0 8px;font-size:22px;line-height:1.3;color:rgb(255,255,255)">{{TITLE}}</h1>
      <p style="margin:0;font-size:13px;color:rgb(170,170,204)">Nate B. Jones &nbsp;·&nbsp; Published: <strong style="color:rgb(224,224,255)">{{DATE}}</strong> &nbsp;·&nbsp; {{PLATFORMS}} &nbsp;·&nbsp; <span style="display:inline-block;padding:2px 9px;border:1px solid rgb(74,108,247);border-radius:10px;font-size:11px;letter-spacing:1px;text-transform:uppercase;color:rgb(170,190,255)">{{LENS_BADGE}}</span></p>
    </td></tr>
    {{SECTIONS}}
  </tbody></table>
</td></tr></tbody></table>
...
**Section partials** (drop into `{{SECTIONS}}` in the lens order above):

```html
<!-- PARAGRAPH section: The Big Idea·Why It Matters Now·Nate's Read·any single-statement section -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 12px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">{{HEADING}}</h2>
  <p style="margin:0;font-size:16px;line-height:1.7;color:rgb(34,34,34)">{{PARAGRAPH}}</p>
</td></tr>

<!-- BULLETED section: His Argument·Receipts·The Contrarian Edge·The Framework·How It Works·Where It Applies·What The Report Says -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 12px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">{{HEADING}}</h2>
...
<!-- ACTION block (highlighted): What It Means For You -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238);background:rgb(248,249,255)">
  <h2 style="margin:0 0 14px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">What It Means For You</h2>
...
<!-- IN HIS WORDS block (1-2 verbatim quotes w/ timestamps) -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 12px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">In His Words</h2>
  <blockquote style="margin:0 0 12px;padding:4px 0 4px 16px;border-left:4px solid rgb(74,108,247);font-style:italic;font-size:15px;line-height:1.7;color:rgb(34,34,34)">“{{QUOTE}}” <span style="font-style:normal;color:rgb(136,136,136);font-size:12px">— {{TIMESTAMP}}</span></blockquote>
</td></tr>

<!-- SOURCE block (always last) -->
<tr><td style="padding:22px 36px 28px">
  ...
  <p style="margin:0 0 8px;font-size:14px;color:rgb(51,51,51)"><strong>Video:</strong> <a href="{{URL}}" style="color:rgb(74,108,247)" target="_blank">{{TITLE}}</a> &nbsp;·&nbsp; {{DURATION}}</p>
  <p style="margin:0 0 8px;font-size:13px;color:rgb(102,102,102)"><strong>Substack:</strong> <a href="{{SUBSTACK_URL}}" style="color:rgb(74,108,247)" target="_blank">{{SUBSTACK_TITLE}}</a> {{SUBSTACK_NOTE}}</p>
  <p style="margin:0 0 8px;font-size:13px;color:rgb(102,102,102)"><strong>Chapters:</strong> {{CHAPTERS}}</p>
...
**Quality bar (BINDING for this routine):** every section must be grounded in the transcript (and the Substack preview), not title/description guesswork. "His Argument" must be his actual reasoning chain; "Receipts" must be the specific data/examples/companies he cites (e.g. a GDPval %, a named firm); "The Contrarian Edge" must capture where he genuinely breaks from the consensus take; "In His Words" must be exact quotes. Never fabricate quotes, numbers, or frameworks — if the transcript is unavailable, ground on the Substack preview + description and OMIT the quote block.

### Implementation Playbook — design tokens (Nate Herk)

Nate Herk's lane is **hands-on implementation**, so his email is a **reproducible playbook**, not a strategy brief — and it is built from his **actual transcript** (spoken words), not just the title/description. It reuses the shared card chrome (dark header `rgb(26,26,46)`, Georgia, 700px card, red Mistake heading, highlighted action block) and adds four things a plain brief lacks:

1. **A video-type badge** in the header — `BUILD`, `DEEP-DIVE`, or `COMMENTARY` (see classification below). The section set adapts to the type.
2. **A Stack strip** under the header (light band `rgb(248,249,255)`) — monospace chips for the tools / models / MCP servers / platforms he uses. Build & Deep-dive only; omit for Commentary.
3. **A Prompts/Commands/Configs code block** — dark `rgb(26,26,46)` / `rgb(224,224,255)` monospace `<pre>` carrying the verbatim prompts, slash-commands, CLAUDE.md snippets, or node settings he states. (Source = what he says on screen + linked resources, not OCR of his editor — say so in the small italic note.)
4. **An "In His Words" block** — 1–2 direct transcript quotes with timestamps. This is the "actual spoken words" signal; every playbook must carry at least one real quote.

The "Steal This For Your Org" action block is highlighted `rgb(248,249,255)`; the "Mistake to Avoid" heading is red `rgb(192,57,43)`; links `rgb(74,108,247)`. Footer identical to the Strategy Brief.

**Video-type classification (pick ONE from the transcript + title):**

| Type | When | Section order |
|------|------|---------------|
| **BUILD** | He builds/automates something step by step | What He Builds → Stack strip → How It Works (Architecture) → Build It Yourself → Prompts, Commands & Configs → Gotchas & Watch-Outs → Steal This For Your Org → Mistake to Avoid → In His Words → Source & Resources |
| **DEEP-DIVE** | He explains / ranks / tests a tool or feature | What It Is → Stack strip → How It Works → The Verdict → When To Use It (and When Not) → Set It Up → Gotchas & Watch-Outs → Steal This For Your Org → Mistake to Avoid → In His Words → Source & Resources |
| **COMMENTARY** | His take on news/a release/a report (no build) | What Happened → Nate's Take → His Reasoning (with receipts) → What It Means For Builders → What To Watch Next → Steal This For Your Org → Mistake to Avoid → In His Words → Source & Resources |

**Implementation Playbook HTML skeleton.** Assemble the **header chrome**, then the ordered **section partials** for the chosen type, then close. Fill `{{PLACEHOLDERS}}`; repeat inner `<li>` as needed.

**Header chrome** (Stack strip row is build/deep-dive only — delete that `<tr>` for Commentary):
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:rgb(244,244,244);padding:32px 0;font-family:Georgia,serif"><tbody><tr><td align="center">
  <table width="700" cellpadding="0" cellspacing="0" style="max-width:700px;border-radius:8px;overflow:hidden;font-family:Georgia,serif"><tbody>
    <tr><td style="background:rgb(26,26,46);padding:28px 36px 22px">
      ...
      <h1 style="margin:0 0 10px;font-size:22px;line-height:1.3;color:rgb(255,255,255)">{{TITLE}}</h1>
      <p style="margin:0;font-size:13px;color:rgb(170,170,204)">Nate Herk &nbsp;·&nbsp; Published: <strong style="color:rgb(224,224,255)">{{DATE}}</strong> &nbsp;·&nbsp; {{DURATION}} &nbsp;·&nbsp; <span style="display:inline-block;padding:2px 9px;border:1px solid rgb(74,108,247);border-radius:10px;font-size:11px;letter-spacing:1px;text-transform:uppercase;color:rgb(170,190,255)">{{TYPE_BADGE}}</span></p>
    </td></tr>
    <tr><td style="background:rgb(248,249,255);padding:12px 36px;border-bottom:1px solid rgb(238,238,238)">
      <p style="margin:0;font-size:12px;line-height:1.9;color:rgb(51,51,51)"><strong style="text-transform:uppercase;letter-spacing:1px;font-size:11px;color:rgb(26,26,46)">Stack:</strong> {{STACK_CHIPS}}</p>
    </td></tr>
    {{SECTIONS}}
  </tbody></table>
</td></tr></tbody></table>
...
Stack chip (repeat inside `{{STACK_CHIPS}}`):
```html
<span style="display:inline-block;margin:2px 4px 2px 0;padding:2px 9px;background:rgb(26,26,46);color:rgb(224,224,255);border-radius:4px;font-family:'Courier New',monospace;font-size:12px">{{TOOL}}</span>
```

**Section partials** (drop into `{{SECTIONS}}` in the order from the table above):

```html
<!-- BULLETED section: What He Builds·What It Is·How It Works·Gotchas·What It Means For Builders·When To Use·His Reasoning·What To Watch Next -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 12px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">{{HEADING}}</h2>
  <ul style="margin:0;padding-left:20px;font-size:15px;line-height:1.8;color:rgb(51,51,51)">
    <li style="margin-bottom:10px"><strong>{{POINT}}:</strong> {{DETAIL}}</li>
...
<!-- PARAGRAPH section: Nate's Take·The Verdict·any single-statement section -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 12px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">{{HEADING}}</h2>
  <p style="margin:0;font-size:16px;line-height:1.7;color:rgb(34,34,34)">{{PARAGRAPH}}</p>
</td></tr>

<!-- NUMBERED steps: Build It Yourself·Set It Up (reproducible, grounded in the transcript) -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 14px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">{{HEADING}}</h2>
  <ol style="margin:0;padding-left:20px;font-size:15px;line-height:1.8;color:rgb(51,51,51)">
    <li style="margin-bottom:12px"><strong>{{STEP}}</strong> — {{STEP_DETAIL}}</li>
  </ol>
</td></tr>

<!-- CODE block: Prompts, Commands & Configs (BUILD/DEEP-DIVE) -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 8px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">Prompts, Commands &amp; Configs</h2>
  <p style="margin:0 0 10px;font-size:12px;font-style:italic;color:rgb(102,102,102)">From what Nate says on screen + linked resources — not OCR of his editor.</p>
  <pre style="margin:0;background:rgb(26,26,46);color:rgb(224,224,255);font-family:'Courier New',monospace;font-size:13px;line-height:1.6;padding:16px;border-radius:6px;white-space:pre-wrap;word-break:break-word">{{CODE}}</pre>
</td></tr>

<!-- ACTION block (highlighted): Steal This For Your Org -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238);background:rgb(248,249,255)">
...
<!-- IN HIS WORDS block (1-2 verbatim transcript quotes) -->
<tr><td style="padding:24px 36px 20px;border-bottom:1px solid rgb(238,238,238)">
  <h2 style="margin:0 0 12px;font-size:15px;text-transform:uppercase;letter-spacing:1px;color:rgb(26,26,46)">In His Words</h2>
  <blockquote style="margin:0 0 12px;padding:4px 0 4px 16px;border-left:4px solid rgb(74,108,247);font-style:italic;font-size:15px;line-height:1.7;color:rgb(34,34,34)">“{{QUOTE}}” <span style="font-style:normal;color:rgb(136,136,136);font-size:12px">— {{TIMESTAMP}}</span></blockquote>
</td></tr>

<!-- SOURCE & RESOURCES block (always last) -->
<tr><td style="padding:22px 36px 28px">
  ...
  <p style="margin:0 0 8px;font-size:14px;color:rgb(51,51,51)"><strong>Video:</strong> <a href="{{URL}}" style="color:rgb(74,108,247)" target="_blank">{{TITLE}}</a> &nbsp;·&nbsp; {{DURATION}}</p>
  <p style="margin:0 0 8px;font-size:13px;color:rgb(102,102,102)"><strong>Chapters:</strong> {{CHAPTERS}}</p>
  <p style="margin:0 0 8px;font-size:13px;color:rgb(102,102,102)"><strong>Linked:</strong> {{RESOURCE_LINKS}}</p>
  <hr style="border:none;border-top:1px solid rgb(238,238,238);margin:18px 0">
  <p style="margin:0;font-size:11px;color:rgb(153,153,153)">Generated by AI-Fluent Leader pipeline &nbsp;·&nbsp; {{DATE}} &nbsp;·&nbsp; <a href="https://github.com/moinuddinmusbik/ai-fluent-leader" style="color:rgb(170,170,170)" target="_blank">ai-fluent-leader wiki</a></p>
</td></tr>
```

**Quality bar (BINDING for this routine):** every section must be grounded in the transcript, not the title/description guesswork. "Build It Yourself" must be steps a reader could actually follow; "Prompts, Commands & Configs" must contain real artifacts he names (if he names none, say "No verbatim prompts shown — see linked resources" rather than inventing them); "In His Words" must be exact quotes. Never fabricate code, commands, or quotes.
...
│   ├── transcripts/       # Full video transcripts (Nate Herk playbook — actual spoken words)
